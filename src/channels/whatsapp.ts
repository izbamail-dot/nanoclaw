    ...
    this.sock.ev.on('messages.upsert', async ({ messages }) => {
      for (const msg of messages) {
        ...
        if (groups[chatJid]) {
          ...
          // Detect TTS trigger phrase and activate voice reply mode for this chat
          if (!isBotMessage && isTTSTrigger(content)) {
  ...
  async sendMessage(jid: string, text: string): Promise<void> {
    ...
    try {
      ...
      if (this.ttsMode.has(jid)) {
        ...
        const audioPath = await synthesizeSpeech(prefixed);
        if (audioPath) {
          ...
          logger.info({ jid }, 'Voice message sent (TTS)');
          return;
        }
